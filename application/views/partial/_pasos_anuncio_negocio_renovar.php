...
<div id="contenedor_publicar_anuncio_negocio" class="contenedor_publicar_anuncio" style=" display:;">
                            ...
                            <table class="tabla_pago"  width="500" style=" margin-left:167px;">
                                ...
                                <?php if($this->session->userdata('tipoUsuario') == 3):?>
                                   <?php foreach ($planes as $plan): ?>
                                    <tr>
                                        <td>
                                            <label>
                                                <input data-vigencia="<?php echo intval($plan->vigencia / 30) ?>" data-descuento="<?php echo '0.00' ?>" data-precio="<?php echo '0.00' ?>" type="radio" name="plan_form" class="validate[required]" value="<?php echo $plan->paqueteID ?>" id="RadioGroup2_0" />
                                ...
                                <?php foreach ($planes as $plan): ?>
                                    <tr>
                                        <td>
                                            <label>
                                                <input data-vigencia="<?php echo intval($plan->vigencia / 30) ?>" data-descuento="<?php echo '0.00' ?>" data-precio="<?php echo '0.00' ?>" type="radio" name="plan_form" class="validate[required]" value="<?php echo $plan->paqueteID ?>" <?=($publicacion->paqueteID == $plan->paqueteID) ? '' : 'disabled="disabled"'?> id="RadioGroup2_0" />
    ...
    <script>
        ...
        $(function() {
            ...
            $('[name=plan_form]').on('change', function() {
                ...
                var total_val = 0;//field.data('precio') - (field.data('precio') * (field.data('descuento') / 100));
            ...
            $('#form_anuncio_negocio').submit(function(e) {
                ...
                var form = $(this);
                $.ajax({
                    url: '<?php echo base_url('directorio/renovarDir') ?>',
                    type: 'post',
                    dataType: 'html',
                    data: form.serialize(),
